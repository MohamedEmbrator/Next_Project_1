...
const SingleArticlePage = async (props: any) => {
  ...
  const article: SingleArticle = await getSingleArticle(props.params.id);
  return (
    <section className="min-h-[calc(100vh_-_156px)] container m-auto w-full px-5 pt-8 md:w-3/4">
      ...
      <div className="mt-7">
        {payload ? (<AddCommentForm articleId={article.id} />) : (<p className="text-blue-600 md:text-xl ">You Should Login to Write a Comment</p>)}
      </div>
      <h4 className="text-xl text-gray-800 ps-1 font-semibold mb-2 mt-7">
        Comments
      </h4>
      {article.comments.map((comment) => (<CommentItem key={comment.id} comment={comment} userId={payload?.id}/>))}
    </section>
  );
}
